...
ENABLEBANKING_APP_ID        # Enable Banking application id (control panel) — optional: app boots fine without it, provider disabled at runtime (see below)
ENABLEBANKING_PRIVATE_KEY   # RSA private key, PEM PKCS8 ("-----BEGIN PRIVATE KEY-----"); literal "\n" allowed — optional, same as above
ENABLEBANKING_REDIRECT_URL  # callback URL registered for the Enable Banking application — required only if the two above are set; must be an ABSOLUTE URL
...
- **Enable Banking specifics**: unlike GoCardless (fronted by an external `gocardless-integrator` microservice), `EnableBankingService` calls `api.enablebanking.com` directly. Auth is a per-request `Authorization: Bearer <JWT>` — RS256, signed in-app with the `ENABLEBANKING_PRIVATE_KEY` (PKCS8 PEM; literal `\n` in the env var are normalized to real newlines before parsing), cached in memory until shortly before expiry (`kid` header = `ENABLEBANKING_APP_ID`). The `aud` claim must be emitted as a plain string (`.audience().single(...)`, not `.add(...)` which produces an array) — Enable Banking rejects the array form with `401 "JWT audience is not valid"`. Link flow has an extra step versus GoCardless: `startLink()` opens the bank consent redirect (no `providerReference` yet); `completeLink()` exchanges the callback `code` for a session (`session_id` → persisted on `Account.requisitionId`) plus the authorized accounts. `getTransactions()` paginates via `continuation_key` (capped at `MAX_TRANSACTION_PAGES=50`, logs a warning if the cap is hit — no silent truncation). A 401/403 from `/accounts/{uid}/transactions` is translated into `BankReauthRequiredException` (parent of `GocardlessRequisitionExpiredException`, so `AccountService`'s catch block handles both providers uniformly) via the same Spring Retry `@Recover` gotcha as GoCardless (see below) — a dedicated `@Recover(BankReauthRequiredException, ...)` just rethrows. **`EnableBankingAggregationProvider.getInstitutions()` encodes `institutionId` as `"<aspspName>|<country>"`** since Enable Banking identifies an ASPSP by name+country, not a single id like GoCardless — `startLink()` splits on `|`; the frontend must pass the `id` back verbatim, not reconstruct it. **Optional provider, graceful fallback**: `EnableBankingService.init()` (`@PostConstruct`) never throws — if `ENABLEBANKING_APP_ID`/`ENABLEBANKING_PRIVATE_KEY` are blank or the key fails to parse, it logs a warning and sets `configured=false` instead of failing application startup (GoCardless-only deployments are unaffected). `isConfigured()` exposes this flag. `EnableBankingAggregationProvider.requireConfigured()` is the single choke point checked at the top of all 5 `BankAggregationProvider` methods — when unconfigured it throws `ResponseStatusException(503, ...)`, which `BankingController`'s `/api/banking/enable-banking/...` endpoints surface directly via the existing `GlobalExceptionHandler` (no per-endpoint handling needed). `EnableBankingService.currentToken()` also has a defensive `configured` guard as defense-in-depth, but it should be unreachable in practice since the provider-level check runs first. Full setup walkthrough (control panel registration, RSA key generation, troubleshooting the 401/422 pitfalls above): `docs/ENABLE_BANKING_SETUP.md`.
